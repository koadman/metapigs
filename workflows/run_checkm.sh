...
#
# > qsub -l select=1:mem=2g:ncpus=1 -v METAPIGS_REPO=$HOME/metapigs,GENOME_FOLDERS=genomes.txt
#
# The genome folders are expected to contain the binned FASTA files for each metagenome
#
##############################################################################################
...
#   - conda cache dir
#
nextflow -C $METAPIGS_REPO/workflows/checkm.config run $METAPIGS_REPO/workflows/checkm.nf \
	-resume \
	-with-report \
        -profile cluster \
        --genomes $GENOME_FOLDERS \

...
read_sets = Channel.fromPath(params.run_table)
		...
		.combine(target_db).map{it -> [it[0], it[1], it[2..-1]]}
...
process mmseqs_all {
	...
	mmseqs search ${run}.querydb agly.db ${run}.resultdb tmp --threads ${task.cpus}
	mmseqs convertalis ${run}.querydb agly.db ${run}.resultdb ${run}.resultdb.m8 --threads ${task.cpus}
	"""
}

...
for count, well in sorted((value, key) for (key,value) in well_counts.items()):
    if pool_sample_count[cur_pool] >= samples_per_pool or (cur_pool > 5 and pool_minmax[cur_pool][0]*max_relrange < count):
        # start a new pool
        cur_pool += 1
        pool_sample_count[cur_pool] = 0
        pool_read_count[cur_pool] = 0
...
for pool in pool_samples:
    ...
    pipette.pick_up_tip(p10rack.wells('A1'))
	p10.transfer(
		10,
		pool_from,
		pool_dest,
		disposal_vol=0,
		mix_before=(3), # mix 2 times

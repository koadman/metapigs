...
# Subset to df containing rows with n number of NAs in cols
NROW(ccc)
ccc <- ccc[rowSums( is.na(ccc) ) <=80, ]
NROW(ccc)
...
count_data <- data.frame(ccc_sorted2[,-1], row.names=ccc_sorted2[,1], check.names = FALSE)
...
count_data_2=as.matrix(count_data_2)
...
count_data_2 <- count_data_2[ rowSums(cpm(count_data_2)>100) >= 2, ]
...
fit2 <- eBayes(fit2)
aaa_13 <- topTable(fit2, n = 20, coef="N1vC1", adjust.method = "BH")
...
aaa_13 <- aaa_13[!rownames(aaa_13) %in% rownames(bbb), ]
NROW(aaa_13)
View(aaa_13)
...
# Estimate dispersion
#count_data= estimateGLMCommonDisp(count_data, design, verbose=TRUE)
# Calc normalization
#count_data = calcNormFactors(count_data)
#y <- DGEList( counts=count_data, group=Group)
#fit <- glmQLFit(count_data, design)
...
save.image(file = "~/Desktop/bins_clustering_parsing_dataframes/20191023_12.RData")
load("~/Desktop/bins_clustering_parsing_dataframes/20191023_12.RData")